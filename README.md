...
- `lezioni/` — capitoli: string matching esatto (KMP e $Z$, Boyer–Moore, Aho–Corasick, suffix tree e Ukkonen, Karp–Rabin, Shift–And, LCA), matching inesatto (allineamento, Landau–Vishkin), algoritmi randomizzati (RandQS, Min-Cut, RandAuto, 2-SAT, catene di Markov, random walk su grafi). In appendice Weisfeiler–Leman, che è negli appunti ma è fuori programma (ma interessante)
